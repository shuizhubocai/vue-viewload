...
var _util = {
     ...
     */
    getPicInfo: function getPicInfo(options) {
        var src = options.src || '',
            ...
            rollpolling = function rollpolling() {
...
var VueViewload = function () {
     ...
     * @attr  event                 支持的事件
     */
    function VueViewload(options) {
        _classCallCheck(this, VueViewload);
    ...
    _createClass(VueViewload, [{
        key: 'inView',
        value: function inView(ele) {
            var isInView = false,
        ...
        value: function bindUI() {
            ...
            this.event.forEach(function (item, index) {
                _this2.container.addEventListener(item, _this2.delayRender, false);
                if (_this2.container !== window && item == 'resize') {
                    window.addEventListener(item, _this2.delayRender, false);
                }
            });
        }
        ...
        key: 'unbindUI',
        value: function unbindUI() {
            var _this3 = this;

            this.event.forEach(function (item, index) {
                _this3.container.removeEventListener(item, _this3.delayRender, false);
                if (_this3.container !== window && item == 'resize') {
                    window.removeEventListener(item, _this3.delayRender, false);
                }
            });
        }
        ...
        value: function render() {
            ...
            var _loop = function _loop(_i) {
                var item = _this4.selector[_i];
                if (!item.ele.getAttribute('data-src')) {
                    item.ele.setAttribute('data-src', item.src);
                    item.ele.setAttribute('data-status', _this4.status[0]);
                }
                if (!item.ele.getAttribute('src')) {
                    item.ele.setAttribute('src', _this4.defaultPic);
                }
                if (getComputedStyle(item.ele, null).display == 'none') {
                    _this4.selector.splice(_i--, 1);
                    return 'continue';
                }
                if (_this4.inView(item.ele)) {
                    if (item.ele.nodeName.toLowerCase() == 'img') {
                        _util.getPicInfo({
                            src: item.src,
                            errorCallback: function errorCallback(options) {
                                item.ele.src = _this4.errorPic;
                                item.ele.setAttribute('data-status', _this4.status[2]);
                            },
                            loadedCallback: function loadedCallback(options) {
                                if (_this4.effectFadeIn) {
                                    item.ele.style.opacity = 0;
                                }
                                item.ele.src = options.isError ? _this4.errorPic : item.src;
                                item.ele.removeAttribute('data-src');
                                item.ele.setAttribute('data-status', _this4.status[1]);
                                setTimeout(function () {
                    ...
                    _this4.callback(item.ele, item.src);
                    _this4.selector.splice(_i--, 1);
                }
                i = _i;
            };
    ...
    install: function install(Vue) {
        ...
        Vue.directive('view', {
            bind: function bind(el, binding) {
                var containerName = binding.arg == undefined ? 'window' : binding.arg;
...
var _vueViewload = __webpack_require__(0);

var _vueViewload2 = _interopRequireDefault(_vueViewload);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

_vue2.default.use(_vueViewload2.default, {
